...
def train(args, unmix, encoder, device, train_sampler, criteria, optimizer):
    # unpack encoder object
    nsgt, insgt, cnorm = encoder
    mse_criterion, sdr_criterion = criteria
...
def valid(args, unmix, encoder, device, valid_sampler, criteria):
    # unpack encoder object
    nsgt, insgt, cnorm = encoder
    mse_criterion, sdr_criterion = criteria
    ...
    with torch.no_grad():
        ...
        for x, y in pbar:
            ...
            for i, X_block in enumerate(X):
                Y.append(transforms.phasemix_sep(X_block, Ymag_hat[i]))

            y_hat = insgt(Y, length=y.shape[-1])
...
def main():
    ...
    criteria = mse_criterion, sdr_criterion
    ...
    for epoch in t:
        ...
        train_loss = train(args, unmix, encoder, device, train_sampler, criteria, optimizer)
        valid_loss = valid(args, unmix, encoder, device, valid_sampler, criteria)
        ...
        if tboard_writer is not None:
            tboard_writer.add_scalar(f'{args.target} loss (MSE)/train', train_loss, epoch)
            tboard_writer.add_scalar(f'{args.target} SI-SDR/valid', valid_loss, epoch)
