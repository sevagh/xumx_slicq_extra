...
def wiener(
    ...
    * Refinining these initial estimates through a call to
    ...
    if softmask:
        ...
        print(f'mix_stft: {mix_stft.shape}')
    ...
    nb_samples, nb_channels, nb_f_bins, nb_slices, nb_t_bins, _ = mix_stft.shape

    mix_stft = torch.flatten(mix_stft, start_dim=-3, end_dim=-2)
    y = torch.flatten(y, start_dim=-4, end_dim=-3)

    mix_stft = mix_stft.permute(0, 3, 2, 1, 4).to(torch.float64)
    y = y.permute(0, 3, 2, 1, 4, 5).to(torch.float64)

    # call expectation maximization per sample, per slice
    for i in range(nb_samples):
        y[i, ...] = expectation_maximization(y[i, ...], mix_stft[i, ...], iterations, eps=eps)[0]

    # scale estimates up again
    y = y * max_abs

    y = y.permute(0, 3, 2, 1, 4, 5)
    y = y.reshape(nb_samples, nb_channels, nb_f_bins, nb_slices, nb_t_bins, -1, nb_sources).to(torch.float32)
    return y
