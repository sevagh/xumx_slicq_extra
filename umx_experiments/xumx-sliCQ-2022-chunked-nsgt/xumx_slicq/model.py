...
class OpenUnmixTimeBucketCDAE(nn.Module):
    def __init__(
        ...
        nb_samples, nb_channels, nb_f_bins, nb_frames = slicq_sample_input.shape
        ...
        for i in range(layers,0,-1):
            if i == 1:
                # last layer is a sigmoid with no batch normalization
                decoder.append(
                    Sequential(
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x_shape = x.shape
        nb_samples, nb_channels, nb_f_bins, nb_frames = x_shape
