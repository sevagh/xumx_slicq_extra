...
from .nsgt import NSGT, BarkScale, MelScale, LogScale, VQLogScale, OctScale
...
class NSGTBase(nn.Module):
    def __init__(self, scale, fbins, fmin, N, fs=44100, device="cuda", gamma=25., matrixform=False):
        ...
        self.device = device
        self.nsgt = NSGT(self.scl, fs, N, real=True, multichannel=True, device=self.device)
    ...
    def predict_input_size(self, batch_size, nb_channels, seq_dur_s):
        ...
        x = torch.rand((batch_size, nb_channels, int(seq_dur_s*self.fs)), dtype=torch.float32, device=self.device)
        shape = x.size()
...
class TorchINSGT(nn.Module):
    ...
    def forward(self, X_list) -> Tensor:
