...
def train(args, unmix, stft, istft, cnorm, device, train_sampler, optimizer):
...
def valid(args, unmix, stft, istft, cnorm, device, valid_sampler):
    losses = utils.AverageMeter()
    unmix.eval()
    with torch.no_grad():
        pbar = tqdm.tqdm(valid_sampler, disable=args.quiet)
        for x, y in pbar:
            pbar.set_description("Validation set")
            x, y = x.to(device), y.to(device)
            X = cnorm(stft(x))
            Y_hat = unmix(X)
            Y = cnorm(stft(y))
...
def get_statistics(args, stft, cnorm, dataset):
...
def main():
    ...
    stft, istft = transforms.make_filterbanks(
        n_fft=args.nfft, n_hop=args.nhop, center=True, sample_rate=train_dataset.sample_rate
    )
    ...
    if args.model or args.debug:
        ...
        scaler_mean, scaler_std = get_statistics(args, stft, cnorm, train_dataset)
    ...
    torchinfo.summary(unmix, input_size=(args.batch_size, args.nb_channels, f_dim, t_dim))
    ...
    for epoch in t:
        ...
        train_loss = train(args, unmix, stft, istft, cnorm, device, train_sampler, optimizer)
        valid_loss = valid(args, unmix, stft, istft, cnorm, device, valid_sampler)
