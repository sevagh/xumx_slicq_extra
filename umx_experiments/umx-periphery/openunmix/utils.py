...
def predict_input_size(rate: float, n_fft: int, n_hop: int, seq_dur: float) -> np.ndarray:
    """Predict input size from seq_dur, fs, nfft and nhop

    Assuming lapped transforms such as STFT

    Args:
        rate (int): Sample rate
        n_fft (int): FFT length and stft window size
        n_hop (int): hop size
        seq_dur (float): sequence duration

    Returns:
        tuple (int, int): time x frequency dimension
    """

    n_frames = int((seq_dur*rate)/(n_fft-n_hop))
    n_freqs = n_fft // 2 + 1
    return (n_frames, n_freqs)
