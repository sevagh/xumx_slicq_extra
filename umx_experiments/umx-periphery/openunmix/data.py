...
def load_datasets(
    ...
    if args.dataset == "aligned":
        ...
        parser.add_argument("--interferer-files", type=str)
        parser.add_argument(
            "--random-track-mix",
            action="store_true",
            default=False,
        ...
        dataset_kwargs = {
            "root": Path(args.root),
            "interferer_files": args.interferer_files.split(','),
...
class FixedSourcesTrackFolderDataset(UnmixDataset):
    ...
    def __getitem__(self, index):
        ...
        track_path = self.tracks[track_index]["path"]
        min_duration = self.tracks[track_index]["min_duration"]
