...
def load_datasets(
    ...
    if args.dataset == "aligned":
        ...
        valid_dataset = MUSDBDataset(
            split="valid", samples_per_track=1, seq_duration=args.seq_dur, **dataset_kwargs
        )
...
class MUSDBDataset(UnmixDataset):
    ...
    def __getitem__(self, index):
        ...
        # pre-mixed musdb track
            ...
            # chain some sequences for validation
            # 1 minute song
            factor = 10

            print('sq dur: {0}'.format(self.seq_duration))

            track.chunk_duration = factor*self.seq_duration
            # set random start position
            track.chunk_start = random.uniform(0, track.duration - factor*self.seq_duration)
