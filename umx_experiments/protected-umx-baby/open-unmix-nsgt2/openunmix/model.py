...
class OpenUnmix(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = Linear(nb_channels*self.nb_bins, hidden_size, bias=False)
        ...
        self.fc3 = Linear(
            ...
            out_features=nb_channels*self.nb_bins,
            bias=False,
        )
        ...
        if input_mean is not None:
            input_mean = torch.from_numpy(-input_mean[: nb_bins]).float()
        else:
            input_mean = torch.zeros(self.nb_bins)

        if input_scale is not None:
            input_scale = torch.from_numpy(1.0 / input_scale[: nb_bins]).float()
        else:
...
class Separator(nn.Module):
    ...
    def forward(self, audio: Tensor) -> Tensor:
        ...
        for j, (target_name, target_module) in enumerate(self.target_models.items()):
            print('applying model for target: {0}'.format(target_name))
        ...
        mix_nsgt = mix_nsgt.permute(0, 3, 2, 1, 4)
        ...
        targets_nsgt = torch.zeros(
            mix_nsgt.shape + (nb_sources,), dtype=audio.dtype, device=mix_nsgt.device
        )
        for sample in range(nb_samples):
            ...
            while pos < nb_frames:
                ...
                targets_nsgt[sample, cur_frame] = wiener(
                    spectrograms[sample, cur_frame],
                    mix_nsgt[sample, cur_frame],
                    self.niter,
        ...
        targets_nsgt = targets_nsgt.permute(0, 5, 3, 2, 1, 4).contiguous()

        print('performing inverse nsgt')
        estimates = self.insgt(targets_nsgt, length=audio.shape[2])
