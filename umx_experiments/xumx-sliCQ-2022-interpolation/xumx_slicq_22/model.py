...
from torch.nn import Parameter, ReLU, Tanh, BatchNorm2d, Conv2d, ConvTranspose2d, Sequential, Sigmoid, Linear, BatchNorm3d, ModuleList
...
class DeOverlapNet(nn.Module):
    def __init__(
        ...
        # linear layers for the deinterpolation
        deinterp_layers = []
...
class OpenUnmixCore(nn.Module):
    def __init__(
        ...
        self.nb_bins = slicq_sample_input.shape[-2]

        channels = [nb_channels, 25, 55, 75]
        layers = len(channels)-1

        filters = [(7, 43), (5, 21), (3, 13)]
