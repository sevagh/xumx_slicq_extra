...
class NSGTBase(nn.Module):
    def __init__(self, scale, fbins, fmin, sllentrlen=None, fs=44100, device="cuda", gamma=25., matrixform=False):
        ...
        sllen, trlen = self.scl.suggested_sllen_trlen(fs)
        print(f'suggested sllen, trlen: {sllen}, {trlen}')

        if sllentrlen is None:
            self.sllen, self.trlen = sllen, trlen
            print(f'using suggested sllen, trlen: {self.sllen}, {self.trlen}')
        else:
            self.sllen, self.trlen = [int(x) for x in sllentrlen.split(',')]
            print(f'using user-specified sllen, trlen: {self.sllen}, {self.trlen}')
