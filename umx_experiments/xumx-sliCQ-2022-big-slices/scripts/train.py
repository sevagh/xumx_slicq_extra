...
def get_statistics(args, encoder, dataset, time_blocks):
    ...
    for ind in pbar:
        ...
        for i, X_block in enumerate(X):
            nb_samples, nb_channels, nb_f_bins, nb_slices, nb_t_bins = X_block.shape

            # stack slices on samples
            X_block_stacked = X_block.view(-1, nb_channels, nb_f_bins, nb_t_bins)

            X_block_stacked = X_block_stacked.mean(1, keepdim=False).permute(0, 2, 1).mean(0, keepdim=False)
            scalers[i].partial_fit(X_block_stacked)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="slice and transition length (comma-separated)",
    )
    parser.add_argument(
        "--nb-channels",
        type=int,
