...
#@profile
def train(args, unmix, nsgt, insgt, cnorm, loss_fn, device, train_sampler, optimizer):
    ...
    for x, y in pbar:
        ...
        Xmag = cnorm(X)
        Xphase = filtering.atan2(X[..., 1], X[...,  0])
        Y_hat = torch.empty_like(X)

        Ymag_hat = unmix(Xmag)
        #Ymag = cnorm(nsgt(y))

        Y_hat[..., 0] = Ymag_hat*torch.cos(Xphase)
        Y_hat[..., 1] = Ymag_hat*torch.sin(Xphase)

        y_hat = insgt(Y_hat, y.shape[-1])

        loss = torch.nn.functional.mse_loss(y_hat, y)
        #loss = torch.nn.functional.mse_loss(Ymag_hat, Xmag)
...
def valid(args, unmix, nsgt, insgt, cnorm, loss_fn, device, valid_sampler):
    losses = utils.AverageMeter()
    unmix.eval()
    with torch.no_grad():
...
def main():
    ...
    nsgt, insgt = transforms.make_filterbanks(
        sample_rate=train_dataset.sample_rate, device=device
    )
    ...
    for epoch in t:
        ...
        valid_loss = valid(args, unmix, nsgt, insgt, cnorm, loss_fn, device, valid_sampler)
