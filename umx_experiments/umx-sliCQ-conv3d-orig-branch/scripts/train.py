...
def valid(args, unmix, encoder, device, valid_sampler, criterion):
    ...
    with torch.no_grad():
        ...
        for xlong, y in pbar:
            ...
            # split big validation songs to avoid oom and go faster
            for x in torch.split(xlong, 1_000_000, dim=-1):
                X = nsgt(x)
                Xmag = cnorm(X)
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Skip dataset statistics calculation and print shapes for dev purposes",
    )
    parser.add_argument(
        "--print-shapes",
        action="store_true",
        default=False,
        help="Print shapes of tensors passing through network",
    )
