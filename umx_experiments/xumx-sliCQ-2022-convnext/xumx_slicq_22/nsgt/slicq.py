...
#        print "rfbas",self.rfbas/float(self.sl_len)*self.fs
    ...
    def setup_lambdas(self):
        self.fwd = lambda fc: nsgtf_sl(fc, self.g, self.wins, self.nn, self.M, real=self.real, reducedform=self.reducedform, measurefft=self.measurefft, device=self.device)
        self.bwd = lambda cc: nsigtf_sl(cc, self.gd, self.wins, self.nn, self.sl_len ,real=self.real, reducedform=self.reducedform, measurefft=self.measurefft, device=self.device)
    ...
    def forward(self, sig):
        ...
        cseq, maxLg, nfbins = chnmap_forward(self.fwd, f_sliced, device=self.device)
        self.maxLg = maxLg
        self.nfbins = nfbins
