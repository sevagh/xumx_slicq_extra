...
def load_target_models(
    ...
    cnorm = transforms.ComplexNormSliCQT().to(device)
    nsgt, _ = transforms.make_filterbanks_slicqt(nsgt_base)
    ...
    #jagged_slicq = nsgt.overlap_add(nsgt.interpolate(cnorm(jagged_slicq)))
...
def load_separator(
    model_str_or_path: str = _pretrained_model,
    niter: int = 1,
    residual: bool = False,
    ...
    if model_path.exists():
        ...
        xumx_model, model_nsgt = load_target_models(
            model_str_or_path=model_path, sample_rate=enc_conf["sample_rate"], device=device
        )
