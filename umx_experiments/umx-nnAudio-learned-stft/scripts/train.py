...
def train(args, unmix, encoder, device, train_sampler, optimizer, criterion):
    stft, cnorm = encoder
    ...
    for x, y in pbar:
        ...
        X = stft(torch.mean(x, 1, keepdim=True))
        Xmag = cnorm(X)

        # unmix prediction magnitude spectrogram
        Ymag_hat = unmix(Xmag)
        #Y_hat = transforms.phasemix_sep(torch.unsqueeze(X, dim=1), Ymag_hat)

        #y_hat = istft(torch.squeeze(Y_hat, dim=1), onesided=True, length=y.shape[-1])
        #y_hat = torch.unsqueeze(y_hat, dim=1).repeat(1, 2, 1)

        #loss = criterion(y_hat, y)

        Ymag = cnorm(stft(torch.mean(y, 1, keepdim=True)))

        loss = torch.nn.functional.mse_loss(Ymag_hat, Ymag)
...
def get_statistics(args, encoder, dataset):
    ...
    for ind in pbar:
        ...
        X = cnorm(stft(torch.mean(x, 0, keepdim=True))).mean(1, keepdim=False).permute(0, 2, 1)
...
def main():
    ...
    stft = transforms.make_filterbanks(
        n_fft=args.nfft, n_hop=args.nhop, sample_rate=train_dataset.sample_rate, fmax=args.bandwidth, center=True
    )
    ...
    unmix = model.OpenUnmix(
        ...
        nb_channels=1, # nnAudio limitation
    ...
    for epoch in t:
        ...
        train_loss = train(args, unmix, encoder, device, train_sampler, optimizer, criterion)
