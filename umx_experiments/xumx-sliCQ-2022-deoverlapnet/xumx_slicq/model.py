...
from torch.nn import Parameter, ReLU, BatchNorm2d, ConvTranspose2d, Conv2d, Sequential, Sigmoid, Linear
...
class DeOverlapNet(nn.Module):
    def __init__(
        ...
        ola = overlap_add_slicq(slicq_sample_input)

        self.nb_channels = ola.shape[1]
        self.nb_m_bins = slicq_sample_input.shape[-1]
        self.nwin = self.nb_m_bins
        self.hop = self.nwin//2

        # magic 'window' for the deoverlap
        self.deoverlap_window = Sequential(
           Linear(in_features=2*self.nwin, out_features=self.nwin, bias=True),
           Sigmoid()
        )
    ...
    def forward(self, slicq: Tensor, nb_slices: int) -> Tensor:
...
class OpenUnmixTimeBucketCDAE(nn.Module):
    def __init__(
        ...
        self.deoverlap = DeOverlapNet(slicq_sample_input)
