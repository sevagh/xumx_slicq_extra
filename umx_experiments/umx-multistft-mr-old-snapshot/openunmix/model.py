...
from torch.nn import GRU, BatchNorm1d, BatchNorm2d, Parameter, ReLU, Tanh, Conv2d, ConvTranspose2d, Linear
...
class OpenUnmix(nn.Module):
    ...
    def __init__(
        ...
        self.bigwin_layers = torch.nn.ModuleList([
            Conv2d(nb_channels, conv_chans_1, (11, 42), stride=(1, 17)), # downsample frequency to 512, hidden_size
            Tanh(),
            BatchNorm2d(conv_chans_1),
            ConvTranspose2d(conv_chans_1, conv_chans_2, (55, 33), stride=(4, 1)), # upsample time by 4
            Tanh(),
        ...
        self.smallwin_layers = torch.nn.ModuleList([
            Conv2d(nb_channels, conv_chans_1, (17, 13), stride=(4, 1)), # downsample time by 4
            Tanh(),
            BatchNorm2d(conv_chans_1),
            ConvTranspose2d(conv_chans_1, conv_chans_2, (2, 12), stride=1), # even out sizes
            Tanh(),
            BatchNorm2d(conv_chans_2),
        ])
        ...
        self.mr_conv = Conv2d(2*conv_chans_2+1, 1, 1)
        self.mr_act = Tanh()
    ...
    def forward(self, x: Tensor, x_bigwin: Optional[Tensor] = None, x_smallwin: Optional[Tensor] = None) -> Tensor:
        ...
        #print(f'x.shape: {x.shape}')
        x = x.reshape(nb_samples, 1, nb_frames, -1)
        #print(f'x.shape: {x.shape}')
        ...
        x = x.reshape(nb_samples, nb_channels, nb_frames, self.nb_output_bins)
