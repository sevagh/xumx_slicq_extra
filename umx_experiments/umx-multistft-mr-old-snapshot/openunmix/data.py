...
def load_datasets(
    ...
    if args.dataset == "aligned":
        ...
        valid_dataset = MUSDBDataset(
            split="valid", samples_per_track=1, valid_seq_duration=args.valid_seq_dur, **dataset_kwargs
        )
...
class MUSDBDataset(UnmixDataset):
    def __init__(
        ...
        valid_seq_duration: Optional[float] = None,
        ...
        **kwargs,
        ...
        self.is_wav = is_wav
        self.seq_duration = seq_duration
        self.valid_seq_duration = valid_seq_duration
        self.target = target
    ...
    def __getitem__(self, index):
        ...
        elif self.split == "valid" and self.valid_seq_duration:
            for k, source in enumerate(self.mus.setup["sources"]):
                # memorize index of target source
                if source == self.target:
                    target_ind = k
                ...
                dur = min(self.valid_seq_duration, track.duration)
