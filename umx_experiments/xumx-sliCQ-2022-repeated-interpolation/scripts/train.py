...
def loop(args, unmix, encoder, device, sampler, criterion, optimizer, train=True):
    ...
    def custom_sampler():
        for x_mix_long, y_bass_long, y_vocals_long, y_other_long, y_drums_long in sampler:
            yield from zip(
                torch.split(x_mix_long, args.max_samples_valid, dim=-1),
                torch.split(y_bass_long, args.max_samples_valid, dim=-1),
                torch.split(y_vocals_long, args.max_samples_valid, dim=-1),
                torch.split(y_other_long, args.max_samples_valid, dim=-1),
                torch.split(y_drums_long, args.max_samples_valid, dim=-1),
            )

    total = sum([max(1, 1+int(s[0].shape[-1]//args.max_samples_valid)) for s in sampler])
...
def get_statistics(args, dataset, encoder):
    ...
    for ind in pbar:
        ...
        Xmag = transforms.repeated_interpolation(transforms.overlap_add_slicq(cnorm(nsgt(x[None, ...]))))
...
def main():
    ...
    parser.add_argument("--max-samples-valid", type=int, default=4_410_000)
    parser.add_argument(
        "--bandwidth",
        type=float,
        default=16000.,
    ...
    if args.model or args.debug:
        ...
        scaler_mean, scaler_std = get_statistics(args, train_dataset, encoder)
