...
def train(args, unmix, encoder, device, train_sampler, criterion, optimizer):
    # unpack encoder tuple
    stft, istft, cnorm, stft_big, stft_small = encoder
    ...
    for x, y in pbar:
        ...
        Xmag = cnorm(X)
        Xmag_big = cnorm(stft_big(x))
        Xmag_small = cnorm(stft_small(x))

        Ymag_hat = unmix(Xmag, Xmag_big, Xmag_small)

        Y_hat = transforms.phasemix_sep(X, Ymag_hat)
        y_hat = istft(Y_hat, x.shape[-1])
...
def valid(args, unmix, encoder, device, valid_sampler, criterion):
    ...
    with torch.no_grad():
        ...
        for x, y in pbar:
            pbar.set_description("Validation tracks")
            xlong, y = x.to(device), y.to(device)

            yhats = []

            # above 10,000,000 samples ooms on my 3080 ti, so split on it
            for (x, yseg) in zip(torch.split(xlong, SPLIT, dim=-1), torch.split(y, SPLIT, dim=-1)):
                X = stft(x)
                Xmag = cnorm(X)
                Xmag_big = cnorm(stft_big(x))
                Xmag_small = cnorm(stft_small(x))
...
def main():
    ...
    encoder = (stft, istft, cnorm, stft_big, stft_small)
