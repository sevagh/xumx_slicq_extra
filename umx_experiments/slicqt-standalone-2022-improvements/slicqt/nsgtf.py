...
def nsgtf_sl(f_slices, g, wins, nn, M=None, real=False, reducedform=0, device="cpu"):
    ...
    fft = fftp(dtype=dtype)
    ifft = ifftp(dtype=dtype)
...
# non-sliced version
def nsgtf(f, g, wins, nn, M=None, real=False, reducedform=0, device="cpu"):
    ret = nsgtf_sl(torch.unsqueeze(f, dim=0), g, wins, nn, M=M, real=real, reducedform=reducedform, device=device)
