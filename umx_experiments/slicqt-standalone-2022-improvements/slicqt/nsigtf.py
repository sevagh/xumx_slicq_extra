...
def nsigtf_sl(cseq, gd, wins, nn, Ls=None, real=False, reducedform=0, device="cpu"):
    ...
    ifft = irfftp(dtype=dtype) if real else ifftp(dtype=dtype)
...
def nsigtf(c, gd, wins, nn, Ls=None, real=False, reducedform=0, device="cpu"):
    ret = nsigtf_sl(torch.unsqueeze(c, dim=0), gd, wins, nn, Ls=Ls, real=real, reducedform=reducedform, device=device)
