...
def ideal_mixphase(track, nsgt, insgt):
    """
    ...
    X = nsgt(torch.unsqueeze(torch.tensor(track.audio), dim=0))
    ...
    for name, source in track.sources.items():
        ...
        # magnitude of STFT
        src_coef = nsgt(torch.tensor(source.audio))
...
class TrackEvaluator:
    ...
    def oracle(self, scale='cqlog', fmin=20.0, bins=12, gamma=25, sllen=None, reps=1, printinfo=False):
        bins = int(bins)
        ...
        nsgt, insgt = transforms.make_filterbanks(scale=scale, fmin=fmin, fbins=bins, gamma=gamma, sample_rate=44100, device=self.device)
        ...
        for _ in range(reps):
            ...
            for track in self.tracks:
                ...
                N = track.audio.shape[0]
                ests = ideal_mixphase(track, nsgt, insgt)

                med_sdrs_bass.append(fast_sdr(track, ests, target='bass', device=self.device))
                med_sdrs_drums.append(fast_sdr(track, ests, target='drums', device=self.device))
                med_sdrs_vocals.append(fast_sdr(track, ests, target='vocals', device=self.device))
                med_sdrs_other.append(fast_sdr(track, ests, target='other', device=self.device))
