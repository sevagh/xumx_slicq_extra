...
def test_nsgt_fwd_cuda_inv_cpu(audio):
    nsgt_base_cuda = transforms.NSGTBase(device="cuda")
    nsgt_base_cpu = transforms.NSGTBase(device="cpu")

    nsgt, _ = transforms.make_filterbanks(nsgt_base_cuda)
    ...
    _, insgt = transforms.make_filterbanks(nsgt_base_cpu)
