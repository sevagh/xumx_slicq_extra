    ...
    with grad_cm():
        for track_tensor in pbar:
            ...
            with amp_cm_cuda(), amp_cm_cpu():
                ...
                # get waveforms back like so
                #
                #nb_samples = x.shape[-1]
                #Ycomplex_all = transforms.phasemix_sep(Xcomplex, Ymag_ests)
                #y_all = insgt(Ycomplex_all, n_samples)
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="use HAAQI instead of SDSDR loss",
    )
    parser.add_argument(
        "--seq-dur",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=16000.,
        help="network won't consider frequencies above this (<0 = disabled)",
    )
    parser.add_argument(
        "--fscale",
    ...
    criterion = LossCriterion()#use_haaqi=args.haaqi)
