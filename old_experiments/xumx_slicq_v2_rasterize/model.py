...
class OpenUnmixCDAE(nn.Module):
    def __init__(
        ...
        self.deoverlap = Sequential(ConvTranspose2d(nb_channels, nb_channels, (1, 3), stride=(1, 2), bias=True), Sigmoid())
        self.mask = True
    ...
    def forward(self, x: Tensor) -> Tensor:
        # copy original mixed magnitude slicqt
        mix = x.detach().clone()

        #print(f"A: {x.shape}")
        preoverlap_shape = x.shape
        crop_lim = preoverlap_shape[-1]*preoverlap_shape[-2]

        # overlap-add slice-wise to get contiguous spectrogram
        ...
        # learned deoverlap
        for i, layer in enumerate(self.deoverlap):
            x = layer(x)

        #print(f"D: {x.shape} (desired: {preoverlap_shape})")

        # crop if necessary
        x = x[:, :, :, : crop_lim]
        #print(f"E: {x.shape}")

        # reshape into slice-wise after growing coefficients
        x = x.reshape(preoverlap_shape)
        #print(f"F: {x.shape}")
...
class OpenUnmix(nn.Module):
    ...
    def __init__(
        ...
        slicq_interp = overlap_add_slicq(jagged_slicq_sample_input)

        self.umx_vocals = OpenUnmixCDAE(
            slicq_interp,
            input_mean=input_mean,
            input_scale=input_scale,
        )
        self.umx_bass = copy.deepcopy(self.umx_vocals)
        self.umx_drums = copy.deepcopy(self.umx_vocals)
        self.umx_other = copy.deepcopy(self.umx_vocals)
    ...
    def forward(self, x, vocals=True, bass=True, drums=True, other=True) -> Tensor:
        if vocals:
            futures_vocals = torch.jit.fork(self.umx_vocals, x)
        if bass:
            futures_bass = torch.jit.fork(self.umx_bass, x)
        if drums:
            futures_drums = torch.jit.fork(self.umx_drums, x)
        if other:
            futures_other = torch.jit.fork(self.umx_other, x)

        y_vocals = torch.jit.wait(futures_vocals) if vocals else None
        y_bass = torch.jit.wait(futures_bass) if bass else None
        y_drums = torch.jit.wait(futures_drums) if drums else None
        y_other = torch.jit.wait(futures_other) if other else None
