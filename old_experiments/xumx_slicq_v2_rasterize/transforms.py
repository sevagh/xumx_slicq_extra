...
class NSGTBase(nn.Module):
    def __init__(self, scale, fbins, fmin, fs=44100, device="cuda", gamma=25., matrixform='rasterize'):
...
class NSGT_SL(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        nsgt_f, ragged_shapes = self.nsgt.nsgt.forward((x,))
...
class INSGT_SL(nn.Module):
    ...
    def forward(self, X, length: int, ragged_shapes=None) -> Tensor:
        ...
        y = self.nsgt.nsgt.backward(X, length, ragged_shapes=ragged_shapes)

        # unpack batch
        y = y.view(*packed_dims, -1)
        return y
...
class ComplexNorm(nn.Module):
    ...
    def forward(self, spec):
        ...
        # take the magnitude
        magspec = torch.pow(torch.abs(torch.view_as_complex(spec)), self.power)

        # downmix in the mag domain to preserve energy
        if self.mono:
            magspec = torch.mean(magspec, 1, keepdim=True)
