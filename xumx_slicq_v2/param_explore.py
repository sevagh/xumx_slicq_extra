...
class TrackEvaluator:
    ...
    def oracle(self, scale='cqlog', fmin=20.0, fmax=22050, bins=12, gamma=25):
        bins = int(bins)
        ...
        n = NSGTBase(scale, bins, fmin, fmax=fmax, device=self.device, gamma=gamma)
...
def optimize_many(f, params, n_iter, per_target):
    if per_target:
        ...
        fmaxes = list(np.arange(*params['fmax']))
        ...
        for _ in tqdm(range(n_iter)):
            while True: # loop in case we skip for exceeding sllen
                ...
                params_tup = (scale, bins, fmin, fmax, gamma)
