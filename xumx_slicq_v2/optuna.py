...
from xumx_slicq_v2.training import _ComplexMSELossCriterion, _SDRLossCriterion
...
MAX_VALID_SAMPLES = 2 # This limits validation data to avoid OOM, use two smaller tracks
...
def define_model(trial):
    ...
    jagged_slicq, sample_waveform = nsgt_base.predict_input_size(BATCHSIZE, 2, SEQ_DUR)
...
def objective(trial):
    ...
    mse_loss = _ComplexMSELossCriterion()
    sdr_loss = _SDRLossCriterion()
...
if __name__ == "__main__":
    ...
    odash_proc = subprocess.Popen(["optuna-dashboard", SQLITE_DB, "--host", "0.0.0.0", "--port", "6006"])
