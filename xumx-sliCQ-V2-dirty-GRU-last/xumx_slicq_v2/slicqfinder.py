...
def evaluate_single(f, params):
    curr_score_bass, curr_score_drums, curr_score_vocals, curr_score_other = f(scale=params['scale'], fmin=params['fmin'], bins=params['bins'])
    ...
    if per_target:
        ...
        for _ in tqdm(range(n_iter)):
            while True: # loop in case we skip for exceeding sllen
                ...
                curr_score_bass, curr_score_drums, curr_score_vocals, curr_score_other = f(scale=scale, fmin=fmin, bins=bins)
