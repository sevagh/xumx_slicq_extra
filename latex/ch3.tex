...
First, Section \ref{sec:stftslicqtcomp} will describe the differences between the output of the STFT and the sliCQT, and Section \ref{sec:torchslicqdatastructure} will describe how we chose an appropriate data structure to represent the sliCQT in PyTorch.

Next, Section \ref{sec:torchslicq} will show the modifications made to the library to compute the transform with PyTorch on the GPU, so that it can be used in the UMX neural network code. It will describe show how the PyTorch sliCQT data structure differs from the STFT.
...
\subsubsection{Comparing the STFT to the sliCQT}
...
The ragged shape of nonuniform frequency transforms like the NSGT was described in \ref{sec:raggedtf} and shown in Figure \ref{fig:raggedslicqt}. Figure \ref{fig:contraststftslicqt} shows the output of the STFT and an arbitrary ragged nonuniform frequency transform such as the NSGT side-by-side.
...
The sliCQT was described in Section \ref{sec:theoryslicqt} as a realtime implementation of the NSGT, consisting of DFT coefficients computed for the input signal split into overlapping slices, shown in Figure \ref{fig:slicqtukeys}. The sliCQT needs each slice to be overlap-added with 50\% of its previous and next slice to produce a meaningful spectrogram, which was shown in Figure \ref{fig:slicqoverlaps}. This is a characteristic specific to the sliCQT, which outputs DFT coefficients grouped by slices. The NSGT, which is the ancestor of the sliCQT not designed for realtime processing, outputs the meaningful spectrogram directly without requiring any overlap-add. To oversimplify, we can say that the sliCQT outputs a ragged time-frequency transform similar to the NSGT for each slice of the input signal, and that to produce the final spectrogram for the entire signal, the transform of each slice need to be overlap-added with each other.
...
\begin{enumerate}
	\item
		The sliCQT computes ragged time-frequency transforms for each slice of length sllen of the input signal of length $N$, where $N > \text{sllen}$. Each slice of the sliCQT is a ragged spectrogram of DFT coefficients corresponding to that slice of the input signal. These slices need to be overlap-added by 50\% with their adjacent slices to create the final meaningful spectrogram. Note that each slice is ragged, due to the nonuniform time-frequency resolution.
	\item
		The overlap-added sliCQT spectrogram has the same ragged characteristic as each individual slice, due to its nonuniform frequency resolution.
\end{enumerate}
...
When choosing a data structure to represent the sliCQT in PyTorch, we must address the two characteristics described in the previous section, which are the adjacent slices that need to be overlap-added, and the raggedness of the slices and final overlap-added transform from the nonuniform time-frequency resolution.

Due to limitations in the PyTorch library, jagged or ragged tensors (which are the equivalent of irregular or ragged matrices) are not yet supported.\footnote{\url{https://github.com/pytorch/pytorch/issues/25032}} That means that the ragged sliCQT cannot be represented by a single tensor, due to the irregular or ragged shape of the underlying matrix from the different frequency bin groupings by time-frequency resolution.

Therefore, to represent the raggedness of the noniform time-frequency resolution, the data structure chosen was a list of tensors. The list data structure in Python was described in Section \ref{sec:pythonbasics}. Each element of the list will contain a tensor for the groups of frequency bins that have the same time-frequency resolution and can fit in the same tensor or matrix.

As described in the previous section, each matrix in the list which have the same time-frequency resolution have their own independent overlap-add, computed by the width of their matrix, which is distinct due to its temporal framerate. Therefore, each element of the list will contain a 3D tensor. The three dimensions are frequency, time, and slice. The 3D tensor, when overlap-added, is converted to a 2D tensor with dimensions of time and frequency, which is equivalent to the 2D time-frequency dimensions of the STFT. Figure \ref{fig:3dslicqola2d} shows how the list of 3D tensors is converted to a list of 2D tensors by the overlap-add procedure.
