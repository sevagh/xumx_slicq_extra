...
Section \ref{sec:inputrepresentation} will describe how the sliCQT was ported to PyTorch to run on the GPU in a neural network, new frequency scales added, and how sliCQT parameters optimal for music demixing will be chosen. Next, section \ref{sec:neuralnet} will discuss the changes made to UMX to use the sliCQT instead of the STFT in xumx-sliCQ, the replacement of the original Bi-LSTM architecture with a CDAE architecture, and the combining of the four independent target networks into a single model with the X-UMX loss functions. Finally, section \ref{sec:postprocessing} will show how the post-processing Wiener-EM step was adapted for the sliCQT.
...
Next, in section \ref{sec:slicqdecision}, we will discuss the choice to use a convolutional architecture in xumx-sliCQ over the original Bi-LSTM architecture of Open-Unmix. In section \ref{sec:convlayers}, we will show how the STFT-based convolutional denoising autoencoder (CDAE) architectures described in section \ref{sec:cdae} were adapted to work on the ragged sliCQT in xumx-sliCQ.
...
First, the sliCQT must always be overlap-added to go from the 3D sliced datastructure to the familiar 2D time-frequency datastructure. Then, all of the operations of UMX which expect a single rectangular matrix of the STFT need to be modified to work on a list of matrices instead, where each separate matrix in the list consists of a group of frequency bins that share the same temporal framerate, ordered from the lowest to highest frequency bins.

A necessary consideration in the network design is the non-invertibility of the overlap-add procedure, since the original 3D sliced datastructure is needed to synthesize an audio waveform. The neural network needs a strategy to compute a 3D de-overlap-added estimate from the 2D overlap-added spectrogram. This was achieved with a final transpose convolutional layer at the end of the neural network, which will be covered in greater detail in the next section \ref{sec:convlayers}.

\subsubsection{sliCQT neural network design choices}
\label{sec:slicqdecision}

\todo[inline]{
Zero-pad: slicqt dimensionality analysis, big transform = use CDAE papers for low parameter
...
Note that after the encoder-decoder layers for the sliCQT, we need to add a ``de-overlap layer.'' This is the inverse of the slice overlap-add procedure with learnable parameters. It uses a transpose convolution layer with a kernel size of $(1, 3)$ and a stride of $(1, 2)$ where the first dimension is frequency and the second is time. This means that for each frequency bin, the kernel slides and considers three time coefficients to produce six time coefficients. The effect is to double the number of time coefficients, which is a reversal of the 50\% reduction of time coefficients from the overlap-add procedure. Finally, the 2D tensor is reshaped to the original 3D shape. The final activation is a Sigmoid, which is real-valued $\in [0.0, 1.0]$. This makes the network estimate a soft or ratio mask which can then be applied to the input mixed magnitude 3D sliCQT spectrogram.
...
\begin{table}[ht]
	...
	\begin{tabular}{ |l|l|l| }
	 \hline
		\# time coefficients & CDAE layer & De-overlap layer \\
	 \hline
	 \hline
...
\begin{tabular}{ |l|l|l| }
	 \hline
		\# frequency bins & CDAE layer & De-overlap layer \\
	 \hline
	 \hline
...
\end{tabular}
	\vspace{1em}
	\caption{De-overlap (inverse 50\% overlap-add) layer}
...
The data loader of Open-Unmix returns a single target at a time, or an $(x, y)$ pair where $x$ is the mixed waveform and $y$ is the desired target. In xumx-sliCQ, the MUSDB18-HQ data loader was modified to return the mixed waveform and all four targets, $(x, y_{\text{vocals}}, y_{\text{bass}}, y_{\text{drums}}, y_{\text{other}})$. Four copies of the CDAE neural network described in the previous section \ref{sec:convlayers} are created in the xumx-sliCQ training script, and each one is trained using a $(x, y_{\text{target}})$ pair for its source.

To implement the time-domain SDR loss, we use the auraloss Python package.\footnote{\url{https://github.com/csteinmetz1/auraloss}} We used the SI-SDR (scale-invariant SDR) \parencite{roux2018sdr} variant of SDR as the loss function. SI-SDR is a simpler and more robust variant of SDR which improves on some failure cases of SDR \parencite{roux2018sdr}. Notably, there is a scale-invariant factor introduced such that differences in scale (i.e., loudness of the waveform) don't significantly affect the SDR. The SI-SDR loss is shown in equation \eqref{equation:sisdr}:
\begin{align}
...
Contrasted with equation \eqref{equation:sdrinstr} shown for the SDR score in section \ref{sec:evalbss}, we note the addition of the factor $\frac{\hat{x}^{T}x}{||x||^{2}}$ to implement the scale invariance.
