...
\explanation{Added chapter roadmap}
...
\citeauthor{moore} states \hlblu{that:}
...
\explanation{changed glockenspiel paragraph, added sheet music and notes}
...
\begin{figure}[ht]
	\centering
	\subfloat[Glockenspiel waveform]{\includegraphics[width=0.725\textwidth]{./images-gspi/glock_waveform.png}}\\
	\subfloat[\hlblu{Glockenspiel notes}]{\includegraphics[width=\textwidth]{./images-gspi/sheetmusic.png}}
	\caption{\hlblu{Glockenspiel waveform in (a) playing a C minor pentatonic melody with 15 strikes. The struck notes are $C, Eb, G, Bb, C, Eb, G, Bb$, shown in (b)}}
...
The Fourier Transform originated as an integral transform in mathematics, which are a class of ``useful tools for solving problems involving certain types of partial differential equations (PDE), mainly when their solutions on the corresponding domains of definition are difficult to deal with'' \parencite[54]{fourierhistory}. The Fourier Transform was originally introduced by Joseph Fourier in his earlier papers \parencite{fourierhist1, fourierhist2}, and fully expanded and collected in his seminal work on heat \parencite{fourierheat}. The connection of the Fourier Transform to music is described by \citeauthor{fouriermusic}, who state \hlblu{that:}
...
\explanation{rearranged these paragraphs to go FT/CTFT $\rightarrow$ DFT $\rightarrow$ FFT}

\begin{shaded}

The quantities $|X(e^{j\omega})|$ and $\angle X(e^{j\omega})$ are referred to as the magnitude and phase respectively. The Fourier Transform is also referred to as the spectrum, while its magnitude and phase are the magnitude and phase \hlblu{spectra,} respectively \parencite{discretebook}.
...
\explanation{added more detail in captions of Figure \ref{fig:glockdft}}
...
\begin{figure}[ht]
	...
	\caption{\hlblu{DFT of the Glockenspiel waveform, using a low frequency resolution with 2,048 points in (a), and a high frequency resolution with 262,144 points in (b). Note the more detailed frequency components shown in the higher frequency resolution transform in (b)}}
	\label{fig:glockdft}
\end{figure}
...
\explanation{describe why i'm showing these diagrams, relate to above paragraphs}
...
\explanation{improved captions of STFT and Gabor transform diagrams}
...
\begin{figure}[ht]
	...
	\caption{Magnitude spectrograms with the Gabor transform, i.e., the STFT with a Gaussian window. \hlblu{Note that they look identical to the STFT with a Hamming window in} Figure \ref{fig:stfts1}. \hlblu{The different window functions are compared in} Figure \ref{fig:gaussvshamm}.}
	\label{fig:stfts2}
\end{figure}
...
\explanation{connection to next section}
...
\explanation{improved captions of STFT and CQT spectrogram figures}
...
\begin{figure}[ht]
	...
	\caption{STFT magnitude spectrograms of the glockenspiel signal. \hlblu{Three different window sizes are shown in increasing order, demonstrating an increasing frequency resolution (sharper horizontal lines) and decreasing time resolution (blurrier vertical lines).}}
...
\begin{figure}[ht]
	...
	\vspace{-0.35em}
	\caption{CQT magnitude spectrograms of the glockenspiel signal. \hlblu{Three different bins-per-octave are shown in increasing order. The time and frequency resolution is varied within a single spectrogram, unlike the fixed time-frequency resolution of each spectrogram in} Figure \ref{fig:cqtvsstft1}. \hlblu{Using more bins-per-octave increases the highest frequency resolution and decreases the lowest time resolution.}}
...
\textcite{doerflerphd} in their dissertation analyzed music with multiple Gabor dictionaries (\hlblu{i.e.,} STFTs). Figure \ref{fig:dorflertradeoff} shows the time-frequency tradeoff of the short and long window STFT analyses of the glockenspiel signal for its transient and tonal characteristics.
...
\begin{figure}[ht]
	\centering
	\subfloat[Adjacent slices placed side-by-side without overlap-adding]{\includegraphics[width=0.675\textwidth]{./images-gspi/gspi_overlap_flatten.png}}\\
...
\begin{figure}[ht]
	...
	\caption{Illustration of the ragged NSGT, where frequency bins are grouped by their time resolution. \hlblu{The green-colored matrix represents the frequency bins analyzed with the highest frequency resolution and lowest time resolution, resulting in frequencies spaced close together and the lowest number of temporal frames. Moving upwards, the yellow, pink, and blue matrices have a decreasing frequency resolution and increasing time resolution, resulting in an increasing spacing between frequencies and an increasing number of temporal frames.}}
...
\explanation{in caption of ragged transform in Figure \ref{fig:raggedslicqt}, i described the color legend for the temporal frame rates}
...
\explanation{moved quote out of caption of Figure \ref{fig:dspmlmix}}
\begin{shaded}
...
\explanation{say that i will talk about CNN and RNNs in the upcoming sections}
...
\begin{tight_enumerate}
	\item
		They are stored as multi-dimensional arrays (\hlblu{e.g.,} the STFT or sliCQT)
	\item
...
\explanation{added section on pooling and up-sampling, will be important later in CDAE section \ref{sec:cdae}}
...
\explanation{i add an intro for why i'm talking about software, code, git, etc.}
...
\explanation{added sentence to paragraph making more clear the difference between GitHub and Git}
...
\explanation{added explanation/roadmap of topics i will cover in this section}
...
\explanation{removed section on open-source ecosystem, just mentioned it here}
...
\explanation{
re-stated my definition of music source separation vs. demixing from ch1\\
defined what stems are\\
removed mentions of the MDX21 demixing challenge, saving that for the appendix
}
...
A common idea in survey papers is that the domains of speech enhancement and music source separation are both important subproblems of audio source separation of \parencite{musicsepintro1, musicmask}.

I provided definitions for music source separation and music demixing in Section \ref{sec:intro}, to distinguish them from the more general problem of audio source separation:
...
\explanation{added definitions of CASA and BSS}
...
The MUSDB18-HQ dataset has fixed train, validation, and test subsets of data. The dataset is organized into \Verb#train# and \Verb#test# folders, and a subset from the training folder is defined to be the validation set by the Python MUSDB18-HQ loader library.\footnote{https://github.com/sigsep/sigsep-mus-db}\explanationtwo{don't mention 14 tracks}

\subsubsection{Evaluation measures}
\label{sec:evalbss}

\explanation{add an intro paragraph for why we want an objective measure and why i picked BSS from its popularity}
...
\explanation{added intro to why i'm talking about this}
...
\explanation{added a new figure to show masking}
...
\begin{figure}[ht]
       \centering
		...
		\caption[Simple example of applying a time-frequency mask to a spectrogram]{\hlblu{Simple example of applying a time-frequency mask to a spectrogram}\footnote[1]{\url{https://source-separation.github.io/tutorial/basics/tf_and_masking.html}}}
               \label{fig:simplemasks}
       \end{minipage}
\end{figure}

\explanation{described what to look for in soft vs. binary mask in Figure \ref{fig:masks} caption}

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.8\textwidth]{./images-mss/maskdemo.png}
	\caption{Results of a soft and binary oracle mask applied for speech denoising \parencite[71]{masking}. \hlblu{When the binary mask is applied, there is a more dramatic separation, and there is starker contrast between the non-zero and zero parts of the resulting spectrogram. When the soft mask is applied, the separation is more gentle, and a range of zero to non-zero values can be seen in the resulting spectrogram.}}
	\label{fig:masks}
\end{figure}
...
\explanation{added segue to noisy phase/mixed-phase/MPI section since i talk about magnitude vs. phase spectrogram here}
...
\explanation{add diagram showing why phase is not typically modelled in Figure \ref{fig:noisyphase}}
...
\begin{figure}[ht]
       \centering
		...
		\caption[Phase spectrograms of an audio signal on the left and random noise on the right, showing that phase is difficult to model]{\hlblu{Phase spectrograms of an audio signal on the left and random noise on the right}, showing that phase is difficult to model\footnote[1]{\url{https://source-separation.github.io/tutorial/basics/phase.html}}}
...
\explanation{conclude section with a simpler sentence}
...
\explanation{explicitly state how the different time-frequency manipulations of HPSS inspired the hypothesis of this thesis}
...
\explanation{
Rename section from ``Wavelets'' which is more about time-frequency transforms, to ``Source separation with wavelets''\\
Add intro and re-use Bark-scale wavelet paper from a previous section here to reinforce some of the key concepts throughout the thesis
}
...
\explanation{add intro, why i'm studying UMX}
...
\explanation{split masking and mag/phase from dataset paragraph}
...
There are four independent Bi-LSTM neural networks, shown in Figure \ref{fig:umxes2},\explanationtwo{add new figures} for each of the four sources: vocals, bass, drums, and other. The four Bi-LSTM models are trained until the loss stops improving. At this point, UMX is ready to perform inference.
...
\explanation{added two figures on individual UMX training and four independent UMX for inference}
...
\begin{figure}[ht]
	\centering
	\subfloat[UMX, single target training]{\includegraphics[width=0.8\textwidth]{./images-blockdiagrams/umx_new_single.png}}\\
	\subfloat[UMX, four targets inference]{\includegraphics[width=\textwidth]{./images-blockdiagrams/umx_new_multiple.png}}
	\caption{\hlblu{UMX training for a single target and inference for four targets}}
	\label{fig:umxes2}
\end{figure}
...
\explanation{add intro to xumx, why i'm talking about it}
...
\begin{figure}[ht]
	\centering
	\subfloat[Multi-domain loss (MDL), showing the frequency-domain MSE loss from the spectrograms and time-domain SDR loss from the waveforms summed with a mixing coefficient $\alpha$ from equation \eqref{equation:totalloss}]{\includegraphics[width=\textwidth]{./images-neural/xumx1_2.png}}\\
	\subfloat[Combination loss (CL), showing the 14 combinations of the four targets from Table \ref{table:14targets}]{\includegraphics[width=\textwidth]{./images-neural/xumx2.png}}
	\caption{\hlblu{Diagrams of the loss functions} of X-UMX \parencite[2]{xumx}}
...
\begin{figure}[ht]
	\centering
	\subfloat[UMX system for a single target with loss function]{\includegraphics[width=\textwidth]{./images-blockdiagrams/umx_single_target.png}}\\
	\subfloat[X-UMX network for all four targets with MDL and CL loss functions]{\includegraphics[width=\textwidth]{./images-blockdiagrams/xumx_multiple_targets.png}}
	\caption{UMX and X-UMX loss functions compared. \hlblu{In UMX, a copy of the network is trained independently for each target, using single-target spectrogram loss. In X-UMX, four copies of the network are trained simultaneously for all four targets, using multi-domain and combination loss functions as shown in} Figure \ref{fig:xumxlosses}.}
...
\explanation{
improved loss diagram captions referring to equations and tables above in Figure \ref{fig:xumxlosses}\\
verbally describe UMX vs. XUMX in Figure \ref{fig:umxandxumx} caption
}
...
\explanation{
added intro to why i'm talking about CDAEs\\
removed confusing wording, only include one diagram in Figure \ref{fig:cdaes}, simplify whole section
}
...
\begin{figure}[ht]
	...
	\caption{Diagram of a CDAE {\parencite[2]{plumbley1}}, where 2D convolutional layers are applied to the input spectrogram. \hlblu{Max-pooling layers are used in the encoder to reduce the dimensionality, and up-sampling layers are used in the decoder to increase the dimensionality.}}
	\label{fig:cdaes}
\end{figure}
