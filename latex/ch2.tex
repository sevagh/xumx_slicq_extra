...
Spectrogram factorization models are more sophisticated than KAM, and the most popular spectrogram factorization model is Nonnegative Matrix Factorization (NMF) (\cite{musicmask, musicsepgood}). According to \textcite[12]{musicsepgood}, NMF ``attempts to factorize a given nonnegative matrix into two nonnegative matrices,'' and it can be applied to the magnitude spectrogram of the mix, $M$, to separate it into frequency weight matrices $W$ and time activation matrices $H$; a survey on NMF techniques by \textcite{nmfpaper} covers the algorithm in more detail. Most recently, data-driven approaches based on machine learning and deep learning have significantly surpassed past approaches (\cite{musicsepgood, sisec2018}). Figure \ref{fig:spectraldemix} shows different techniques for spectral demixing.
...
\begin{figure}[ht]
	\centering
	\subfloat[Two lattices within a super-tile; the rectangular region is the super-tile, the ellipses represent the domains within which each TF atom's spectrogram exceeds some fixed threshold, and the dots represent their center (the TF sampling points)]{\includegraphics[width=0.475\textwidth]{./images-wavelets/tfjigsaw-supertiles.png}}
	\hspace{0.1em}
	\subfloat[$G^{0}$ and $G^{1}$ represent the analysis maps for the two Gabor frames, $\tilde{G}^{0}_{p}$ and $\tilde{G}^{1}_{p}$ represent the partial synthesis maps from the selected TF atoms, and $\mathcal{H}$ represents the calculation of entropy and corresponding decision]{\includegraphics[width=0.475\textwidth]{./images-wavelets/tfjigsaw-entropycriterion.png}}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=\textwidth]{./images-wavelets/tfjigsaw-sep-example.png}
	\caption{TFJigsawSep for tonal/transient separation}
	\label{fig:tfjigsawdemo}
\end{figure}

The next tonal/transient separation algorithm considered is the WMDCTLasso algorithm (\cite{wmdct}).\footnote{\url{https://homepage.univie.ac.at/monika.doerfler/StrucAudio.html}} The time-frequency transform used is the WMDCT (Windowed Modified Discrete Cosine Transform) applied with two different time-frequency resolutions, by using a wide and narrow window. Next, a method called Group-LASSO (Least Absolute Shrinkage and Selection Operator) is applied to separate the tonal and transient layers by exploiting their respective sparsity in each time-frequency resolution.

In the wide window MDCT analysis, the Group-LASSO shrinkage in the frequency group selects the most sparse coefficients in frequency, representing tonal parts of the signal. In the narrow window MDCT analysis, the  Group-LASSO shrinkage in the time group selects the most sparse coefficients in time, representing transient parts of the signal. An example of a resulting decomposition of the glockenspiel signal is shown in figure \ref{fig:wmdctex}.

\begin{figure}[ht]
	\centering
	\subfloat[WMDCT of glockenspiel signal]{\includegraphics[width=0.425\textwidth]{./images-wavelets/Glock1.jpg}}
	\hspace{0.1em}
	\subfloat[Tonal estimate]{\includegraphics[width=0.425\textwidth]{./images-wavelets/Glock_Ton.jpg}}\\
	\subfloat[Transient estimate]{\includegraphics[width=0.425\textwidth]{./images-wavelets/Glock_Trans.jpg}}
	\hspace{0.1em}
	\subfloat[Reconstructed glockenspiel WMDCT]{\includegraphics[width=0.425\textwidth]{./images-wavelets/Glock_Recon.jpg}}
	\caption{WMDCTLasso for tonal/transient separation of the glockenspiel signal\protect\footnotemark}
	\label{fig:wmdctex}
\end{figure}
...
The STFT and Gabor expansions can be considered as a form of wavelet analysis (\cite{gaborwavelet1, gaborwavelet2}), although the details of the relationship are beyond the scope of this thesis. Still, we keep in mind that wavelet-based source separation techniques are related to the previously shown algorithms that use the STFT, CQT, or other Gabor expansions like the WMDCT.
