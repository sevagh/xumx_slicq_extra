...
One potential source of poor performance in xumx-sliCQ is the design choice of using an independent neural network for each of the sub-matrices of the ragged sliCQT, proposed in Section \ref{sec:slicqarches}. A target (or source) may output frequencies from different sub-matrices of the ragged sliCQT, such that the frequencies of the target are divided among neural networks that are not connected. By contrast, UMX and X-UMX both use a single neural network applied to the STFT, which contains all of the frequencies of all of the targets.

Another potential source of errors in xumx-sliCQ is the de-overlap layer, shown in Figure \ref{fig:cdaeslicqt} and described in Section \ref{sec:deoverlap}. The sliCQT must be overlap-added to create a spectrogram, which is an operation that has no inverse. In xumx-sliCQ, I implemented the inverse overlap with a transpose convolution layer added after the music demixing layers. By contrast, UMX and X-UMX do not need any extra layers, since the STFT can be used as a spectrogram directly.

Finally, the CDAE neural network architecture chosen for xumx-sliCQ was originally designed for the STFT. The CDAE hyperparameters in xumx-sliCQ, shown in Tables \ref{table:convtable1}--\ref{table:convtable4} in Section \ref{sec:slicqarches}, were kept similar to the original STFT-based values (\cite[3]{plumbley1}) (\cite[3]{plumbley2}). Moreover, the same CDAE architecture and hyperparameters were copied for each sub-matrix of frequency bins in the sliCQT. A more sophisticated approach could take into account the nonlinear frequency scale and nonuniform time-frequency resolution of the sliCQT to choose optimal architectures and hyperparameters for each CDAE.
