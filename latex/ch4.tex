...
This \hlblu{chapter} contains the results of the experiments described in Chapter \ref{ch:methodology}, and a discussion of the results. Refer to Appendix \ref{appendix:computerspec} for the hardware and software specifications of the computer which produced the results in this \hlblu{chapter}.

The goal of this thesis \hlblu{is} to use the sliced Constant-Q Transform (sliCQT) in CrossNet-Open-Unmix (X-UMX), a neural network for music demixing. X-UMX represents musical signals with the Short-Time Fourier Transform (STFT), which is limited by a fixed time-frequency resolution. In contrast, the sliCQT has a varying time-frequency resolution, which may be more suitable for musical applications such as music demixing.
...
\explanation{i removed the unnecessary mention of the buggy, off-by-default multithreaded behavior of the library. it's not relevant to the rest of the thesis}
...
In Section \ref{sec:slicqparamsrch}, I described the random search to choose the parameters of the sliCQT to use in xumx-sliCQ. Since the goal of xumx-sliCQ is to try to surpass X-UMX, which uses the STFT, the first step is to \hlblu{try} the sliCQT that has the most potential to surpass the STFT in music demixing. This was done by choosing the best Signal-to-Distortion (SDR) score on the mixed-phase inversion (MPI) oracle waveforms of validation tracks of MUSDB18-HQ. The MPI strategy is when the estimate of the magnitude spectrogram of the target is combined with the phase of the original mixed audio, described in Section \ref{sec:noisyphaseoracle}. The MPI strategy is used in both X-UMX and xumx-sliCQ.
...
\begin{table}[ht]
	...
	\begin{tabular}{ |p{2.5cm}|l|l|p{4cm}|p{4cm}| }
	 ...
	 \hline
		\makecell[l]{xumx-sliCQ \\ sliCQT-WEM} & slicq-wslicq & 3.7 & \url{https://github.com/sevagh/xumx-sliCQ} & \url{https://github.com/sevagh/xumx-sliCQ/tree/main/pretrained-model} \\
	 \hline
...
xumx-sliCQ scored a median SDR of 3.6 dB with the STFT Wiener-EM step, and 3.7 dB with the sliCQT Wiener-EM step. xumx-sliCQ failed in its objective to improve the music demixing performance of UMX and X-UMX with both of its post-processing strategies.

One potential source of poor performance in xumx-sliCQ is the design choice of using an independent neural network for each of the sub-matrices of the ragged sliCQT, proposed in Section \ref{sec:slicqarches}. A target (or source) may output multiple frequencies that are in different sub-matrices of the ragged sliCQT, such that the time-frequency information of the target is divided among the independent neural networks which have no connections with each other. By contrast, UMX and X-UMX both use a single neural network applied to the STFT, which contains all of the time-frequency information of all of the targets.

Another potential source of poor performance in xumx-sliCQ is the de-overlap layer, shown in Figure \ref{fig:cdaeslicqt} and described in Section \ref{sec:deoverlap}. The sliCQT must be overlap-added to create a spectrogram, which is an operation that has no exact inverse. In xumx-sliCQ, I implemented the de-overlap procedure with a transpose convolution layer added after the music demixing layers, shown in Figure \ref{fig:cdaeslicqt}. By contrast, UMX and X-UMX do not need any extra layers, since the STFT is can be used as a spectrogram directly.

	Finally, the CDAE neural network architecture chosen for xumx-sliCQ was originally designed for the STFT \parencite{plumbley1, plumbley2}. In Section \ref{sec:replacestft}, I described how the STFT-based neural network code was adapted to the different data structure of the sliCQT. However, the convolution kernels sizes shown in Tables \ref{table:convtable1}--\ref{table:convtable4} in Section \ref{sec:slicqarches} were unchanged from the original STFT-based values (\cite[3]{plumbley1}, \cite[3]{plumbley2}).
