...
The execution time of the transform improved with PyTorch. The GPUs have an advantage over the CPU in the matrix form of the transform, but not in the ragged form. This can be explained by the fact that the design of the ragged transform, which is a list of tensors which each contains a bucket of frequency bins with the same time-frequency resolution, requires looping over lists which negates some of the benefits of GPU parallelism.

Even though the GPU is not faster than the CPU for the tested parameters of ragged sliCQT, having the transform on the GPU allows us to use the sliCQT inside a neural network with the deep learning features of PyTorch.
...
To maximize result reproducibility, even though table \ref{table:bsseval} describes how to download the pretrained models of UMX, X-UMX, and xumx-sliCQ from their respective hosted locations, all the pretrained models and code were copied in a separate public repository to generate the results above.\footnote{\url{https://gitlab.com/sevagh/xumx_slicq_extra/-/tree/main/mss_evaluation}}
...
\subsection{Neural network size and inference performance}
