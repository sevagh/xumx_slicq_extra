...
\hlblu{STFT-based music demixing techniques may be limited by the fixed time-frequency resolution of the STFT. The objective of this thesis is to replace the STFT in a music demixing model with the sliCQT, which has a varying time-frequency resolution. Open-Unmix} \parencite{umx} \hlblu{is a state-of-the-art deep neural network for music demixing based on the STFT, with an open-source reference implementation}.\footnote{\url{https://github.com/sigsep/open-unmix-pytorch}} \hlblu{The goal of this thesis is to adapt Open-Unmix to create a usable music demixing software application based on the sliCQT.}
