...
, where $\hat{x}[n]$ is the quantized sample. Quantization levels can be defined to be uniform (evenly spaced) or nonuniform, and in essence the sample values are rounded to the nearest quantization level $2^{B}$, recalling that $B$ is the number of bits per sample in the representation (\cite{discretebook}).
...
\begin{figure}[ht]
	...
	\subfloat[Waveform results of the ADC process]{\includegraphics[width=0.675\textwidth]{./images-tftheory/adc3.png}}\\
