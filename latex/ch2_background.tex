...
We close the chapter with a quote from \textcite{doerflersouls} on the powerful properties of the STFT:
...
\textcite{invertiblecqt} implemented the constant-Q transform with Nonstationary Gabor frames (\cite{balazs}), which allows for perfect reconstruction for the first time. This is also called the CQ-NSGT, or constant-Q Nonstationary Gabor Transform. The next section, \ref{sec:theorynsgt}, will cover the NSGT, but for now we shall continue with how the NSGT led to improved implementations of the CQT.
...
In the landscape of music analysis libraries, Essentia,\footnote{\url{https://essentia.upf.edu/}} (\cite{essentia}), LTFAT,\footnote{\url{https://ltfat.org/}} (\cite{ltfat}) and the MATLAB Wavelet Toolbox\footnote{\url{https://www.mathworks.com/help/wavelet/ref/cqt.html}} have converged on the CQ-NSGT (\cite{invertiblecqt, slicq, variableq1}). However, librosa\footnote{\url{https://librosa.org/}} (\cite{librosa}) still uses the older implementation with the approximate reconstruction (\cite{klapuricqt}). Finally, \textcite{slicq} have released an open-source reference Python implementation\footnote{\url{https://github.com/grrrr/nsgt}} of the CQ-NSGT and the sliCQT.
...
\begin{figure}[ht]
	\centering
	\subfloat[CQT spectrogram, 12 bins-per-octave]{\includegraphics[width=0.475\textwidth]{./images-tftheory/glock_cqt12.png}}
	\subfloat[STFT spectrogram, window size 256]{\includegraphics[width=0.475\textwidth]{./images-tftheory/glock_stft256.png}}\\
	\subfloat[CQT spectrogram, 24 bins-per-octave]{\includegraphics[width=0.475\textwidth]{./images-tftheory/glock_cqt24.png}}
	\subfloat[STFT spectrogram, window size 1024]{\includegraphics[width=0.475\textwidth]{./images-tftheory/glock_stft1024.png}}\\
	\subfloat[CQT spectrogram, 48 bins-per-octave]{\includegraphics[width=0.475\textwidth]{./images-tftheory/glock_cqt48.png}}
	\subfloat[STFT spectrogram, window size 4096]{\includegraphics[width=0.475\textwidth]{./images-tftheory/glock_stft4096.png}}
...
The advantage of the CQT over such approaches is that it contains these desirable transform properties in one single transform. The NSGT is the next evolution of the CQT based on frame theory (\cite{frametheory}). To derive the NSGT, let's recall the previously-seen definition of the Gabor transform from section \ref{sec:jointtfa}. In the standard Gabor transform, the same window function (also called the Gabor atom or Gabor function) is shifted in time to cover entire signal (\cite{adaptivecqt}), described by equation \ref{equation:stationarygab}:
...
Continuing from their summary of \textcite{doerflerphd}'s work on multiple Gabor frames, \textcite{adaptivecqt} continue on to describe the outcomes of the Nonstationary Gabor frames of \textcite{jaillet, balazs}: ``[t]he nonstationary Gabor frames [...] are a further development; [...] they provide for a class of FFT-based algorithms [...] together with perfect reconstruction formulas.''
...
From these NSGT equations, we can get back the stationary Gabor transform by setting a fixed time constant $a$ or frequency constant $b$ in the equations \ref{equation:irregulartime} and \ref{equation:irregularfrequency} respectively. Finally, figure \ref{fig:nonuniformtflattices} show both the cases of the varying resolution by time and by frequency.

\begin{figure}[ht]
	\centering
	\subfloat[Gabor atoms that change with time]{\includegraphics[width=5cm]{./images-tftheory/irregulartime.png}}
	\hspace{1em}
	\subfloat[Gabor atoms that change with frequency]{\includegraphics[width=5cm]{./images-tftheory/irregularfrequency.png}}
	\caption{Varying time-frequency resolution of the Nonstationary Gabor transform}
