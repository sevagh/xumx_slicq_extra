...
The number of points, or samples, of the DFT, determines the frequency resolution, also called $\mathit{df}$ or $\mathit{\Delta f}$, which is the frequency spacing between each point in the resulting spectrum (\cite{discretebook}). The frequency resolution of an $N$-point DFT is $\mathit{df} = \frac{F_{s}}{N}$, where $F_{s}$ is the sampling rate of the input signal. A longer DFT therefore has a finer frequency resolution with more points.  An illustration of the complex DFT spectrum along with its magnitude and phase spectra are shown in figure \ref{fig:glockdft}, computed using the FFT implementation of the DFT available in MATLAB, using two different lengths of DFT to show the difference in the lower and higher frequency resolutions.
...
\begin{figure}[ht]
	...
	\caption{DFT of the Glockenspiel waveform, computed with the FFT}
	\label{fig:glockdft}
\end{figure}
...
\chaptertodo{
describe the unit impulse as an infinite time pulse contrast with Fourier which is an infinite frequency\\
...
\textcite{gabor1946}'s seminal signal processing paper, \textit{The Theory of Communication}, introduced significant and far-reaching concepts in the time and frequency analysis of acoustic signals. Gabor starts from the Fourier transform, which as described in section \ref{sec:freqanal} is a tool that can express a time-domain signal as a sum of infinite complex sinusoids containing the frequency components present in the signal.

In his paper, Gabor quotes famed American telecommunication engineer John Carson to describe the limitations of the Fourier transform:
...
According to \textcite{korpel}, ``Gabor came to the conclusion that the difficulty lay in our mutually exclusive formulations of time analysis and frequency analysis ... he suggested a new method of analyzing signals in which time and frequency play symmetrical parts.'' We can begin from figure \ref{fig:gaborfirst}, showing the mutually exclusive formulations of time and frequency. Figure \ref{fig:gaborfirst}(a) shows the unit impulse, which is a theoretical 

\begin{figure}[ht]
	\centering
	\includegraphics[width=5cm]{./images-tftheory/gabor2.png}
	\caption{tf intuition}
	\label{fig:tfintuition}
\end{figure}

We refer the reader to \cite{carsonfamous} for a summary of Carson's accomplishments. From various papers describing the significance of Gabor's works, we can list his important contributions:

\begin{enumerate}
	\item
	\item
		Gabor derived the principal of time-frequency uncertainty stemming from Heisenberg's uncertainty principle in quantum physics, leading to an explosion of wavelet research in the 1980s (\cite{homage1})
	\item
		Gabor was the first to propose that any signal of finite energy can be decomposed into a linear combination of time-frequency shifts of the Gaussian function -- this was the first known use of the short-time Fourier transform, local Fourier transform, or windowed Fourier transform (\cite{dictionary})
	\item
		The short-time Fourier transform was rediscovered independently of Gabor (\cite{stftindie}), but is increasingly referred to as the Gabor transform (\cite{dictionary})
	\item
		Gabor showed that the use of the Gaussian window in the short-time Fourier transform minimizes time-frequency uncertainty (\cite{gabor1946})
\end{enumerate}
...
%	\begin{quote}
%	\vspace{2em}
%		Gabor came to the conclusion that the difficulty lay in our mutually exclusive formulations of time analysis and frequency analysis ... he suggested a new method of analyzing signals in which time and frequency play symmetrical parts.\cite{korpel}
%
