...
\chaptertodo{
Summarize CDAE papers (should be simple)\\
...
The proposed adaptation of Open-Unmix (UMX) (\cite{umx}) to use the sliCQ Transform (sliCQT) (\cite{invertiblecqt, slicq}) was named xumx-sliCQ, and is the subject and main result of this thesis. The following methodology section will cover how xumx-sliCQ was written step-by-step.

Manipulating time-frequency resolution is one strategy for improving spectral music source separation. Open-Unmix uses a fixed time-frequency resolution from the STFT with a window size of 4096. \textcite{fitzgerald1, driedger} use multiple STFTs with different window sizes, and \textcite{fitzgerald2, cqtseparation, bettermusicsep} replaced the STFT with the constant-Q transform.

Open-Unmix (UMX) is a deep neural network which uses a bi-directional LSTM network to estimate the magnitude STFTs for 4 targets: bass, vocals, drums, and other. It is the hypothesis of this thesis that using a sliCQT spectrogram, which can contain more musical information than the STFT from its adaptive time-frequency resolution, will allow the Open-Unmix network to perform music demixing more effectively.

Open-Unmix is a published academic paper (\cite{umx}), but it can also be considered an open-source framework for music demixing. The project on GitHub\footnote{\url{https://github.com/sigsep/open-unmix-pytorch}} uses PyTorch (\cite{pytorch}), a GPU-capable deep learning and numerical computation library for Python. It also conveniently includes a MUSDB18-HQ data loader (\cite{musdb18hq}) and BSS metrics evaluator (\cite{bss}), which are the dataset and evaluation metrics for music demixing being considered in this thesis.

xumx-sliCQ will be based on this template, and as a result, all of the code developed will be in Python and with PyTorch code targeting the GPU. The rest of this section will be focused on the Python and PyTorch implementation details of xumx-sliCQ. There are three requirements to make the thesis objective possible:
...
\begin{tight_enumerate}
	...
	\item
		Use the sliCQT in Open-Unmix, in section \ref{sec:slicqumx}
\end{tight_enumerate}

\subsection{PyTorch NSGT and sliCQT}
