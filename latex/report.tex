...
Musical signals have characteristics that lead to conflicting requirements in the STFT \textcite{doerflerphd}. In the low frequency region, music needs to be analyzed with long-duration windows for a high frequency resolution, as bass notes lay the harmonic basis of a song \todo{heavy citation stuff here}. Conversely, the high frequency region contains transients and broadband sounds, which are useful for timbre identification and rhythm. These transients have fast attacks and decays and need to be analyzed with short-duration windows.
...
\subsubsection{Acoustic signals and their digital representation}

\ichfeedback{is this a good section?}

\todo[inline]{acoustic signals, pressure waves, etc.}

\todo[inline]{discrete time, shannon-nyquist sampling theorem}

\subsubsection{Fourier transform and frequency analysis}
