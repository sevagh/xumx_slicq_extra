...
**The realtime model scores 4.07 dB and is light and fast!** It takes an average of 2 seconds to demix a song with the PyTorch model with a GPU, and 11 seconds with a CPU.<sup>†</sup> Further speedups are provided in the form of ONNXRuntime for (CPU, 5 seconds) and TensorRT (CUDA GPU, 0.037s).
...
| xumx_slicq_v2.export | Export model to TensorRT, Onnxruntime | CPU |
