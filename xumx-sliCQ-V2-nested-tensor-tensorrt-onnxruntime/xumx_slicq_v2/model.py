...
class Unmix(nn.Module):
    ...
    def forward(self, Xcomplex, return_masks=False) -> Tensor:
        nb_tf_blocks = Xcomplex.size(0)
        Ycomplex = [None]*nb_tf_blocks
        Ymasks = [None]*nb_tf_blocks

        for i in range(nb_tf_blocks):
            Xblock = Xcomplex.unbind()[i]
...
class _SlicedUnmix(nn.Module):
    ...
    def forward(self, xcomplex: Tensor, x: Tensor) -> Tensor:
        ...
        ret = torch.zeros((4, nb_samples, nb_channels, nb_f_bins, nb_slices, nb_t_bins,), device=x.device, dtype=x.dtype)
        ret_masks = torch.zeros((4, nb_samples, nb_channels, nb_f_bins, nb_slices, nb_t_bins,), device=x.device, dtype=x.dtype)
