...
class NSGT_SL(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return torch.nested.nested_tensor(C)
...
class ComplexNorm(nn.Module):
    ...
    def forward(self, spec):
        nb_tf_blocks = spec.size(0)
        nb_samples = spec.size(1)
        nb_channels = spec.size(2)
        nb_slices = spec.size(4)

        # nested tensor irregular dims
        # nb_f_bins = spec.size(3)
        # nb_t_bins = spec.size(5)

        ret = [None]*nb_tf_blocks

        for i in range(nb_tf_blocks):
            C_block = torch.pow(torch.abs(torch.view_as_complex(spec.unbind()[i])), 1.0)
            ret[i] = C_block

        return torch.nested.nested_tensor(ret)
